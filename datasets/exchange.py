...
def _process_data(missing_rate, look_window, forecast_window, stride_window, SAVE_PATH):
    ...
    X_times = pd.read_csv(PATH + '/data/exchange/exchange.csv')
...
def get_data(missing_rate, look_window, forecast_window, stride_window, note):
    ...
    loc = base_base_loc / ('exchange_' + str(note) + '_seq_' + str(look_window) + '_pred_' + str(forecast_window) + '_stride_' + str(
            stride_window) + '_missing_' + str(missing_rate))
    SAVE_PATH = '/processed_data/exchange_' + str(note) + '_seq_' + str(look_window) + '_pred_' + str(
        forecast_window) + '_stride_' + str(stride_window) + '_missing_' + str(missing_rate)
    PATH = os.path.dirname(os.path.abspath(__file__))
