...
class CPLSTM(torch.nn.Module):
    ...
    def forward(self, args, x, coeffs, times):
        ...
        if len(X0.shape) > 3:
            X0=X0.squeeze(-1)
            rev_X0.squeeze(-1)
        if torch.isnan(self.initial.weight).any() or torch.isnan(self.initial.bias).any():
            print("initial layer parameters contain NaNs")
            print("Weight:", self.initial.weight)
            print("Bias:", self.initial.bias)
            raise ValueError("initial layer parameters are NaN")
        z0 = self.initial(X0)
